...
public class SBService {
		...
		public void BonbongService() {
			...
			switch (no) {
				case 1:
					...
					/// 같은 이름의 직급있으면 걸러내기
					bVO = dao.getSearchB(jikkub);
					if (bVO.getJikkub() != null) {
						System.out.println("이미 존재하는 직급입니다. \n* * 등록 실패 * *");
						return;
					}

					bVO.setJikkub(jikkub);

					System.out.print("해당 직급( " + jikkub + " )의 급여 : ");
					bVO.setBonbong(scanner.nextInt());
					res = dao.setInputB(bVO);
					if (res == 0)
				...
				case 3:
					...
					for (int i = 0; i < vos.size(); i++) {
						bVO = vos.get(i);
						System.out.print("\t  " + bVO.getJikkub() + "\t");
						System.out.print("\t\t  " + bVO.getBonbong() + "\n");
					}
					System.out.println("-".repeat(60));
      ...
      switch (no) {
          case 1://입력
              ...
              while(true) {
                  ...
                  name=scanner.next();
                  ////////////동명이인 비교(같은이름안받음)////////
                  sVO=dao.getSearchS(name);
                  if(sVO.getName()!=null) {
                      System.out.println("이미 같은 이름이 존재합니다.\n작업을 종료합니다.");
                      return;
                  }else sVO.setName(name);
                  ...
                  sVO.setJikkub(jikkub);
                  ///////////////////직급입력끝////////////////
                  System.out.print("특근시간 : ");
                  sVO.setNight(scanner.nextInt());
                  
                  //////////////////입력완료/////////
                  int res=dao.setInputS(sVO);
          ...
          case 2://수정
              ...
              if(sVO.getName()==null) {
                  ...
                  System.out.println("-".repeat(60));
                  System.out.println("사  번\t: "+sVO.getSabun());
                  System.out.println("이  름\t: "+sVO.getName());
                  System.out.println("직  급\t: "+sVO.getJikkub());
                  System.out.println("특근시간\t: "+sVO.getNight()+" 시간");
              }
              boolean run=true;
              String newName = sVO.getName();
              jikkub = sVO.getJikkub();
              int night = sVO.getNight();
              ...
              re:while(run) {
                  ...
                  if(res==0)System.out.println("수정에 실패했습니다. . .");
          ...
          case 4://전체조회
              ...
              for(int i=0; i<vosS.size(); i++){
                  sVO=vosS.get(i);
                  System.out.print(sVO.getSabun()+"\t");
                  System.out.print(sVO.getName()+"\t");
                  System.out.print(sVO.getJikkub()+"\t");
                  System.out.print(sVO.getNight()+" 시간\t");
                  /////사원의 직급과 비교하고 해당직급의 본봉 출력하기/////////
                  for(int j=0; j<vosB.size(); j++) {
                      bVO=vosB.get(j);
                      if(sVO.getJikkub().equals(bVO.getJikkub())) {
                          calc(sVO, bVO.getBonbong());
                          //계산용..
                          System.out.print(bVO.getBonbong()+"\t");
                      }
                  }
                  //////////////////////////////////////////////
                  System.out.print(sVO.getTax()+"\t");
              ...
              while(true) {
                  ...
                  if(sVO.getName() != null) {
                      ...
                      for(int i=0; i<vosB.size(); i++) {
                          bVO=vosB.get(i);
                          if(sVO.getJikkub().equals(bVO.getJikkub())) {
                              calc(sVO, bVO.getBonbong());	//계산용..
                              break;
                          }
                      }
                      /////////////////////////////
                      System.out.print("본  봉\t:"+sVO.getBonbong() + "\n");
                      System.out.println("공제액\t:"+sVO.getTax()+"\t");
                      System.out.println("실수령액\t:"+sVO.getSalary()+"\n");
  ...
  // 급여 계산 처리..
  private void calc(SawonVO sVO, int bonbong) {
      int sudang=(sVO.getNight()*25000);
      //voS.setBonbong(voB.getBonbong());
      sVO.setBonbong(bonbong);
      sVO.setTax((int)((sVO.getBonbong()+sudang)*0.1) );
      sVO.setSalary(sVO.getBonbong()+sudang-sVO.getTax());
  }
}