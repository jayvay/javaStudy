...
insert into test1 values (default, '부원장선생님',50);	-- age 추가 후 삽입한 레코드라서 age 따로 추가해줘야 함
...
insert into test1 values (default, '철수', 5, '서울');
...
-- 철수의 주소를 '떡잎마을'로 변경
update test1 set address = '떡잎마을' where name = '철수';
...
update test1 set gender = '남자' where name = '신형만';
...
update test1 set address = '떡잎유치원' where name in ('나미리','채성아','차은주','원장선생님','부원장선생님');
...
-- 30대 남자만 출력하세요.
select * from test1 where gender = '남자' and age >= 30 and age < 40;

-- 30대 남자를 이름 내림차순으로 출력하세요.
select * from test1 where gender = '남자' and age >= 30 and age < 40 order by name desc;

-- 나이가 30대이거나 50대인 자료만 출력하세요.
select * from test1 where (age >= 30 and age < 40) or (age >= 50 and age < 60);

-- 떡잎마을에 살거나 와르르맨션에 사는 사람을 출력하세요.
select * from test1 where address = '떡잎마을' or address = '와르르맨션';
select * from test1 where address in ('떡잎마을','와르르맨션');

-- 떡잎마을에 살거나 와르르맨션에 사는 사람 중에서 나이 10대 이하만 출력하세요.
select * from test1 where (address = '떡잎마을' or address = '와르르맨션') and age < 20;
...
select * from test1 where name like '신%'; -- '가'로 시작한다는 뜻
...
-- 이름 중에서 두 번째 글자가 '짱'을 포함한 자료를 출력하세요.
select * from test1 where name like '_짱%';	-- _은 첫 번째 글자

-- 이름 중에서 두 번째 글자가 '짱'을 포함한 자료의 '성명/주소'를 출력하세요.
select name,address from test1 where name like '_짱%';	
