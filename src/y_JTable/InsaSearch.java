		...
		btnUpdate.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(age.trim().equals("")) {
					...
					//정상적으로 수정한 자료가 입력되어 넘어 온다면 모든 값을 vo에 담아서 DB에 저장(수정) 처리한다.
...
//				System.out.println(name);	//에러나서 체크할 때는 무조건 찍어봐라. 에러 해결하고 나면 지우세요.
				int ans = JOptionPane.showConfirmDialog(null, name + "을(를) 삭제하시겠습니까?", "회원 삭제", JOptionPane.YES_NO_OPTION);
				if(ans == 0) {
					res = dao.setInsaDelete(name);
					if(res == 0) JOptionPane.showMessageDialog(null, "회원정보 삭제 실패!");
					else {
						JOptionPane.showMessageDialog(null, "회원정보 삭제 완료");
						dispose();
					}
					
				}
				else JOptionPane.showMessageDialog(null, "회원정보 삭제를 취소하셨습니다.");
			}
		});
